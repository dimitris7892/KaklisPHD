...
class TensorFlowW1(BasePartitionModeler):
    ...
    def createModelsFor(self, partitionsX, partitionsY, partition_labels,tri,X,Y):
        ...
        weights = np.array(np.append(weights0.reshape(-1,1),np.asmatrix(weights0).reshape(-1,1),axis=0).reshape(2,-1))
...
class TensorFlowWLSTM(BasePartitionModeler):
        ...
        if len(partition_labels) > 1:
            for idx, pCurLbl in enumerate(partition_labels):
                    ...
                    for i in range(0, len(partitionsX[idx])):
                        ...
                        XSplineClusterVector.append(np.append(partitionsX[idx][i], vectorNew))
...
class TensorFlowWeights(BasePartitionModeler):
    ...
    def createModelsFor(self, partitionsX, partitionsY, partition_labels,tri,X,Y):
        ...
        sModel=[]
        sr = sp.Earth(max_degree=1, max_terms=200,enable_pruning=False)
        sr.fit(X,Y)
        sModel.append(sr)
        import csv
        csvModels = [ ]
            ...
            with open('./trainedModels/model_Gen_.csv', mode='w') as data:
                ...
                for row in modelSummary:
                    ...
                    try:
                        ...
                        #if basis != "MSE:":
                        if pruned=='No':
                            data_writer.writerow([ basis, coeff if coeff!='None' else 1 ])
                            genModelKnots.append(basis)
                    except:
                        x = 0
        ...
        #try:
        #XSplineVector = np.reshape(XSplineVector, (XSplineVector.shape[0], XSplineVector.shape[1], 1))
        estimator.fit(X, XSplineVector, epochs=100, validation_split=0.33,verbose=0)
        #score = estimator.evaluate(np.array(XSplineVector),Y, verbose=0)
        #except:
        ...
        if len(partition_labels) > 1:
            for idx, pCurLbl in enumerate(partition_labels):
                    ...
                    #try:
                    #XSplineClusterVector = np.reshape(XSplineClusterVector, (XSplineClusterVector.shape[0], XSplineClusterVector.shape[1], 1))
                    estimatorCl.fit(np.array(partitionsX[idx]),np.array(XSplineClusterVector),epochs=100,verbose=0)#validation_split=0.33
...
class SplineRegressionModeler(BasePartitionModeler):
    ...
    def getBestModelForPoint(self, point):
        ...
        dBestFit = 0
        # For each model
        for i in range(0,len(self._models ) -1):
            # If it is a better for the point
...
class TensorFlowWLSTM2(BasePartitionModeler):
    ...
    def createModelsFor(self, partitionsX, partitionsY, partition_labels,tri,X,Y):
        ...
        def baseline_model():
            ...
            #model.add(keras.layers.Dense(2+genModelKnots-1, input_shape=(2+genModelKnots-1,)))
            model.add(keras.layers.LSTM(genModelKnots, input_shape=(n_steps,1)))
        ...
        if len(partition_labels) > 1:
            for idx, pCurLbl in enumerate(partition_labels):
                    ...
                    estimatorCl.add(keras.layers.LSTM(16 ,input_shape=(16,1)))
                    #estimatorCl.add(keras.layers.Dense(2 + numOfNeurons - 1, input_shape=(2 + numOfNeurons - 1, )))
                    estimatorCl.add(keras.layers.Dense(numOfNeurons - 1, ))
                    #estimatorCl.add(keras.layers.Dense(2))
                    estimatorCl.add(keras.layers.Dense(1, ))
    ...
    def getFitnessOfModelForPoint(self, model, point):
        return 1.0 / (1.0 + numpy.linalg.norm(np.mean(self._partitionsPerModel[ model ],axis=1) - point))