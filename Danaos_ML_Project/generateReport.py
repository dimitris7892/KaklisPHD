...
class GenerateReport:
    ...
    def __init__(self,company, vessel, server, sid, password, usr):
        ...
        self.weatherCompTemplate = r'''
                        \section{Weather Comparison - Sensor / Weather Service (NOA) - Total }
                                    ...
                                    \begin{figure}[H]
                                        ...
                                        \includegraphics
                                        [width=1\textwidth]                             
                                        {./Figures/%(company)s/%(vessel)s/windSpeedWS_NOA.eps}
        ...
        self.templateLegInfo = r'''\textbf{{\large Leg %(legNumber)i \#  Basic INFO:}}
                                    ...
                                    \begin{itemize}
                                        ...
                                        \begin{itemize}
                                            \footnotesize
                                            {
                                                ...
                                                \item Total Predicted FOC (\textcolor{ForestGreen}{Neural DT}): \textbf{%(totalPredFocDT)s MT}
                                                \item FOC Perc Diff (\textcolor{Blue}{Neural Net}): \textbf{%(percDiff)4.2f %(perc)s}
                                                \item FOC Perc Diff (\textcolor{ForestGreen}{Neural DT}): \textbf{%(percDiffDT)4.2f %(perc)s}
                                            }
                                         \end{itemize}
                                   ...
                                   \begin{figure}[H]
                                    ...
                                    \includegraphics
                                    [width=1\textwidth]                             
                                    {./Figures/%(company)s/%(vessel)s/windSpeedWS_NOAleg%(legNumberFig)s.eps}
                                    \caption{\footnotesize{Wind Speed Sensor VS NOA Distributions comparison }}
                                    \label{fig:ws}
                                    \end{figure}
        ...
        self.overallAcc = r'''     \textbf{{\large Overall Accuracy}}
                            ...
                            \begin{figure}[H]
                                ...
                                \includegraphics
                                [width=1\textwidth] 
                                {./Figures/%(company)s/%(vslFig)s/evalPerf%(vslFig)s.eps}
                                \caption{\footnotesize{FOC Actual vs Predicted / Speed Ranges, Overall on Legs Test Data}}
                                \label{fig:heatmap}
                            \end{figure}
                            ...
                            \begin{figure}[H]
                                ...
                                \includegraphics
                                [width=1\textwidth] 
                                {./Figures/%(company)s/%(vslFig)s/overallAccPerMile.eps}
                                \caption{\footnotesize{FOC/mile Actual vs Predicted / Speed Ranges, Overall on Legs}}
                                \label{fig:heatmap}
                            \end{figure}
        ...
        self.templateSummaryTable = r'''
                            \captionof{table}{%(model)s} 
                            \begin{table}[H]
                                \centering
                                  \caption*{Total Actual FOC \footnotesize{(MT)}, Total Predicted FOC \footnotesize{(MT)},
                                   Percentage Difference(Actual / Predicted), 
                                   Mean STW (knt), Mean Draft (m) for each leg}
                                    \label{tab:mae}
                                \resizebox{\textwidth}{!}{    
        ...
        self.templateROLegInfo =  r'''
                                     \textbf{ Routing Optimization Info :}
                                    ...
                                    \begin{table}[H]
                                       \resizebox{\textwidth}{!}{ %(perc)s
                                          ...
                                          \textbf{BASIC COMPARISON} &
                                              \textbf{Optimization(RETA = True)} &
                                              \textbf{Our Estimation} &
                                              \textbf{Optimization(RETA = False)} \\ \hline
                                             Distance (nm)  & %(distNMOptTrue)s  &  %(distNMEst)s & %(distNMOptFalse)s \\ \hline
                                             Time (hours)   &   %(totalTimeOptTrue)s   & %(totalTime)s  & %(totalTimeOptFalse)s \\ \hline
                                             Avg Speed (kt) &    %(avgSpeedOptTrue)s  &   %(avgSpeed)s & %(avgSpeedOptFalse)s  \\ \hline
                                             Total FOC (MT) &   %(totalFocOptTrue)s   & %(totalFocEst)s  & %(totalFocOptFalse)s  \\ \hline
                                ...
                                \textbf{ Weather comparison optimization / initial:}
                                    ...
                                    \item \textbf{ RestrictETA = False}
                                        
                                         
                                       \begin{figure}[H]
                                                      \centering
                                                      \includegraphics
                                                      [width=%(scaleFalse)s\textwidth]                             
                                                      {./Figures/%(company)s/%(vessel)s/optimized_initial_False_%(legNumber)s.eps}
                                                      \caption{\footnotesize{Wind Speed comparison Initial / Optimized Route, %(leg)s }}
                                                      \label{fig:heatmap}
                                        \end{figure}
        ...
        self.main = r'''
            ...
            \setcounter{page}{1} %% Setzt die Seitenzahl auf 1 zurück		
             ...
             \section{Models Performance Overview}
    ...
    def valCheck(self):
        ...
        if len(sorted(glob.glob(path1 + '*.csv'))) == len(sorted(glob.glob(path2 + '*.csv'))) == len(sorted(glob.glob(path3 + '*.csv')))\
    ...
    def generate(self):
            ...
            #print("Length Leg Files problem")
            #return
        ...
        predDfs, accs, legs, sumActs, sumPreds, percDiffs, stws, drfts, latlonDep, latlonArr = self.extractStatisticsForLegs("", 'stats')

        predDfsDT, accsDT, legs, sumActsDT, sumPredsDT, percDiffsDT, stwsDT, drftsDT, latlonDep, latlonArr = self.extractStatisticsForLegs("NeuralDT", 'stats')
        ...
        #self.extractPlotsForLegs(predDfsPL, predDfsDTPL, legsPL, accsPL, accsDTPL)
        ...
        #self.plotOverallAcc(predDfsOvPL, accsOVPL, predDfsOvDTPL, accsOVDTPL, legsOVPL, legsOVDTPL, predDfsOvGWPL)
        ####END OF PLOTS
        ...
        for i in range(0, len(legs)):
            ######################################################### NN TABLE INFO
            templateRowTableArgs = {'leg': legs[i], 'totalActFoc': "{:,.2f}".format(sumActs[i]), 'totalPredFoc': "{:,.2f}".format(sumPreds[i]),
            ...
            templateRowTableNDTArgs = {'leg': legs[i], 'totalActFoc': "{:,.2f}".format(sumActsDT[i]),
                                    ...
                                    'percDiff': percDiffsDT[i], 'meanSTW': stwsDT[i], 'meanDraft': drftsDT[i], 'perc': '\%'}
            ...
            if totalSailingHoursRO <= 65 and legs[i] != 'SUEZ - SINGAPORE' :
                continue
            ...
            for reta in retas:
                if os.path.isfile('./routingResponses/'+self.vessel+'/mappedResponse_'+self.vessel+'_'+str(reta)+'_'+leg):
                    retCode = 1
                    fromAPI = True
                else:
                    #retCode = -1
            ...
            templateROLegInfoArgs = {'leg':legs[countLegs],
                                        ...
                                        'totalFocOptTrue': "{:,.2f}".format(sumPredsOptFocs[0]) if sumPredsOptFocs[0]!="NA" else sumPredsOptFocs[0],
                                        'totalFocEst': "{:,.2f}".format(sumPredsDT[countLegs]),
                                        'totalFocOptFalse': "{:,.2f}".format(sumPredsOptFocs[1])  if sumPredsOptFocs[1]!="NA" else sumPredsOptFocs[1],

                                         'distNMOptTrue': "{:,.2f}".format(totalDistOpts[0])  if totalDistOpts[0]!="NA" else totalDistOpts[0],
                                         'distNMEst': "{:,.2f}".format(stws[countLegs] * totalSailingTimeHours[countLegs]),
                                         'distNMOptFalse': "{:,.2f}".format(totalDistOpts[1]) if totalDistOpts[1]!="NA" else totalDistOpts[1],


                                         'avgSpeedOptTrue': "{:,.2f}".format(np.mean(stwOpts[0])) if stwOpts[0]!="NA" else stwOpts[0],
                                         'avgSpeed': "{:,.2f}".format(stws[countLegs]),
                                          'avgSpeedOptFalse': "{:,.2f}".format(np.mean(stwOpts[1])) if stwOpts[1]!="NA" else stwOpts[1],

                                         'totalTimeOptTrue': "{:,.2f}".format(totalSailingTimeOpts[0])  if totalSailingTimeOpts[0]!="NA" else totalSailingTimeOpts[0],
        ...
        ######################################### Neural Net #############################################
        templateSummaryTableArgs = {'templateRowTable': templateRowTable,'vslFig': self.vslFig, 'vessel': self.vessel, 'model': 'Neural Net',
                                    ...
                                    'totalDist': "{:,.2f}".format(np.sum(distTravelled)),  'company': self.company, 'perc': '\%'}
        ...
        #########################################> Neural DT <#############################################
        templateSummaryTableArgsNDT = {'templateRowTable': templateRowTableNDT, 'vslFig': self.vslFig, 'model': 'Neural DT',
                                       'vessel': self.vessel,
        ...
        templateOvAccArgs = {'totalInstances': "{:,.0f}".format(instances),
                                    'totalTime': "{:,.0f}".format(np.sum(totalSailingTimeHours)),
                                    'totalDist': "{:,.2f}".format(np.sum(distTravelled)), 'company': self.company, 'vslFig': self.vslFig,
                                  }
    ...
    def extractOverallAccData(self, model, type):
        ...
        rawGW = []
        path = '/home/dimitris/Desktop/KaklisPHD/Danaos_ML_Project/sensAnalysis'+model+'/'+self.vessel+'/'
        predDfs = []
        predDfsGW = []
        legs = []
        accs = []
        ...
        if model =="":
            ...
            raw = np.array(np.append(data[:, 0].reshape(-1, 1),
                                     np.asmatrix([data[:, 1], data[:, 5].astype(float), data[:, 6], focsPerMile, focsPerMilePred]).T, axis=1))
            ...
            raw = np.array(np.append(data[:, 3].reshape(-1, 1),
                                     np.asmatrix([data[:, 4], data[:, 2].astype(float), data[:, 1], focsPerMile, focsPerMilePred]).T, axis=1))
    ...
    def plotOverallAcc(self, predDfs, accs, predfsDT, accsDT, legs, legsDT, predDfsGW):
        ...
        plt.clf()
        for predDf_, predDf_DT, leg, legDT, meanAcc, meanAccDT, predDfGW_ in zip(predDfs, predfsDT, legs, legsDT, accs, accsDT, predDfsGW):
            df = predDf_
            dfDT = predDf_DT
            dfGW = predDfGW_
            ...
            if legDT != "" : ax2 = sns.lineplot(x='speed', y='pred_Foc', data = dfDT, sort=False, color='green',label='neuralDT Foc Acc: '+ meanAccDT)
            ax2 = sns.lineplot(x='speed', y='pred_Foc', data = df, sort=False, color='blue',label='neural Foc Acc: ' + meanAcc)
            ...
            ax2 = sns.lineplot(x='speed', y='focPerMilePred', data=df, sort=False, color='blue',label='neural Foc / Mile Acc: ' + meanAcc)
            ax2 = sns.lineplot(x='speed', y='focPerMile', data=df, sort=False, color='red', label='actual Foc / mile')
            ax2 = sns.lineplot(x='speed', y='focPerMileGW', data=dfGW, sort=False, color='green', label='actual Foc / mile [0 - 3 Bft]')
            ax2.set_ylabel('Foc (MT/day)', fontsize=16)
            ax2.grid()
            plt.title(leg)
    ...
    def extractPlotsForLegs(self,predDfs, predDfsDT, legs, accs, accsDT):
        ...
        for i, predDf_, predDf_DT, leg, meanAcc, meanAccDT in zip(range(0,len(predDfs)) ,predDfs, predDfsDT, legs, accs, accsDT):
            ...
            ax2 = sns.lineplot(x='speed', y='pred_Foc', data = dfDT, sort=False, color='green',label='neuralDT Foc Acc: ' +meanAccDT)
            ax2 = sns.lineplot(x='speed', y='pred_Foc', data=df, sort=False, color='blue',label='neural Foc Acc: ' + meanAcc)
            ax2 = sns.lineplot(x='speed', y='actual_Foc', data=df, sort=False, color='red', label='actual Foc')
            ax2.set_ylabel('Foc (MT/day)', fontsize=16)
            ax2.grid()
    ...
    def extractStatisticsForLegs(self, model, type):
        ...
        #========> NeuralDT ###########################  <========
        return predDfs, accs, legs, sumActs, sumPreds, percDiffs, stws, drfts, latlonDep, latlonArr
    ...
    def getStatsOfOPtimization(self, leg, fromAPI, reta):
        ...
        if fromAPI == True:
            ...
            print("API generated file writen in: " + './APIresp/' + self.vessel + '/compareSpeeddetailed_interp_'+ str(reta)+'_' + leg)
            os.remove(file)
        else:
        ...
        resp = pd.read_csv('./routingResponses/'+self.vessel+'/mappedResponse_' + self.vessel + '_'+ str(reta)+'_'+leg).values
        dist = 0
        totalDist = 0
    ...
    def getOptimizedRouteFromNavigatorForLeg(self, leg, reta):
        ...
        if responseWeatherRouting['foundSol'] == False :
            ...
            with open(self.pathToWebAPIForPavlosBreakRoute + '/response_' + self.vessel + '_' + legWOExt + '.json', 'w') as json_file:
            ...
            with open('./routingResponses/'+self.vessel+'/response_' + self.vessel + '_' +str(reta)+'_'+ legWOExt +'.json', 'w') as json_file:
                json.dump(responseWeatherRouting, json_file)
    ...
    def evalRoutes(self, leg, type, reta):
        ...
        if type == 'optimized':
            ...
            dataLeg = pd.read_csv('./routingResponses/'+self.vessel+'/mappedResponse_'+self.vessel+"_"+str(reta)+'_'+legWOExt+'.csv').values
    ...
    def mapResponseRoute(self, leg, type, reta):
        ...
        if type == 'optimized':
            ...
            f = open('./routingRequestsBody/'+self.vessel+'/request_' + self.vessel + '_'+str(reta)+'_'+ legWOExt +'.json', )
            request = json.load(f)

            f = open('./routingResponses/'+self.vessel+'/response_' + self.vessel + '_' +str(reta)+'_' + legWOExt + '.json', )
            ...
            shutil.copy('/home/dimitris/Desktop/opt2.csv',
                        './routingResponses/'+self.vessel+'/mappedResponse_' + self.vessel + '_' +str(reta)+'_'+ legWOExt +'.csv')
    ...
    def plotOptimizationVSInitialWeather(self, leg, legNum, reta, notAvailable = None):
        ...
        if notAvailable == None:
            routingResp = pd.read_csv('./routingResponses/'+self.vessel+'/mappedResponse_'+self.vessel+'_'+str(reta)+'_'+leg).values

            windSpeedResp = routingResp[:, 2]

            dtResp = list(
                map(lambda x: str(datetime.strptime(x, '%Y-%m-%d %H:%M:%S.%f').replace(second=0, microsecond=0)) if str(x).__contains__('.') else
            ...
            windSpeedInit = np.array([k for k in initialRoute if datetime.strptime(k[8], '%Y-%m-%d %H:%M:%S') in dtResp])
            ...
            #plt.show()
            plt.savefig('./Figures/'+self.company+'/'+self.vessel+"/"+"optimized_initial_"+str(reta)+'_' +str(legNum)+'.eps', format = 'eps')
    ...
    def extractInfoForWeatherComp(self, data, dataCorr, legInd = None):
        ...
        g = sns.displot(data = dictDF, x="Wind Speed (m/s)" , hue="source", )
        ...
        if legInd != None:
            plt.savefig('./Figures/'+self.company+'/'+self.vslFig+'/windSpeedWS_NOAleg'+str(legInd)+'.eps', format='eps')
        else:
            plt.savefig('./Figures/' + self.company + '/' + self.vslFig + '/windSpeedWS_NOA.eps',
                                format='eps')
        ...
        ax.legend()
        if legInd != None:
            plt.savefig('./Figures/' + self.company + '/' + self.vslFig + '/windSpeedCompPerSpeedleg'+str(legInd)+'.eps', format='eps')
        else:
    ...
    def saveResponseRouteCoords(self, leg, reta):
        ...
        for i in range(0,len(path)):
            ...
            lats.append(path[i]['lat'])
            lons.append(path[i]['lon'])

        with open('./coords/' + self.company + '/' + self.vessel +'/'+str(reta)+'_'+legWOExt+'_coords_response.csv', mode='w') as data:
            data_writer = csv.writer(data, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    ...
    def drawRoutesOnMap(self, leg ,type, reta):
        ...
        coordsFileInit = pd.read_csv('./coords/'+self.company+'/' + self.vessel + '/'+str(reta)+'_'+legWOExt+'_coords_.csv').values

        coordsFile = pd.read_csv('./coords/'+self.company+'/' + self.vessel +'/'+str(reta)+'_'+legWOExt+'_coords_'+type+'.csv').values
        ...
        self.plot_path(coordsFileInit[:,0], coordsFileInit[:,1], coordsFile[:,0], coordsFile[:,1], origPoint, destPoint, leg)
    ...
    def buildJsonForRoutingRequest(self, leg, reta):
        ...
        with open('./coords/' + self.company + '/' + self.vessel + '/'+str(reta)+'_'+legWOExt+'_coords_.csv', mode='w') as data:
            data_writer = csv.writer(data, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            data_writer.writerow(['Latitude', 'Longitude'])
            for i in range(0, len(lats)):
                data_writer.writerow(
        ...
        #return
        with open('./routingRequestsBody/'+self.vessel+'/request_'+self.vessel+'_'+str(reta)+'_'+legWOExt+'.json', 'w') as json_file:
