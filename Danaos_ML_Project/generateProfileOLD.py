...
class BaseProfileGenerator:
    ...
    def findCloseToLandDataPoints(self, trData):
        ...
        for i in range(0, len(trData)):
            ...
            if is_in_ocean == False and trData[i, 5] <= 10:
                notIncOcean.append(trData[i])
            elif is_in_ocean == True:
                inOcean.append(trData[i])
        trData1 = notIncOcean

        trData1 = np.array(trData1).astype(float)
    ...
    def fillPORTSCANALSsheet(self, workbook, dtNewPC, json_decoded, company, vessel):
        ...
        while i < maxSpeed:
            ...
            if focArray.__len__() > 0:
                ...
                windSpeed = np.round(np.mean(focArray[:, 4]))
                windDir = np.round(np.mean(focArray[:, 3]))
                swh = np.round(np.mean(focArray[:, 13]),2)
                focJSON = np.round(np.mean(focArray[:, 8]),2)
                item = {"windBFT": int(windSpeed), "windDir": float(windDir), "swell": float(swh), "cons": float(focJSON)}
    ...
    def fillDetailedExcelProfCons(self, company, vessel, pathToexcel, dataSet, rawData, tlgDataset, dataSetBDD,
        ...
        thirdDrft = '(' + str(np.floor(draftsSorted[0][0]) - 1) + ' - ' + str( np.ceil(maxDraft)) + ')'
        ...
        if portsFlag==True:
            workbook._sheets[3].title = 'Cons profile for Ports (' + str(np.floor(draftsSorted[0][0]) - 1) + ' - ' + str(np.ceil(maxDraft)) + ')'
        
        meanDraftBallast = round(float(np.mean(np.array([k for k in ballastDt if k[1] > 0])[:, 1])), 2)
        meanDraftLadden = round(float(np.mean(np.array([k for k in ladenDt if k[1] > 0])[:, 1])), 2)
        meanDraftPorts = round(float(np.mean(np.array([k for k in portsDt if k[1] > 0])[:, 1])), 2)

        velocities = (
            np.array((np.array([k for k in dtNew if float(k[5]) >= 8 and float(k[5]<=30)])[:, 5])).astype(float))  # and float(k[12]) < 18
        ...
        maxVel = np.round(np.max(velocities))
            #np.round(velocitiesSorted[len(velocitiesSorted)-1][0])
            #np.floor(np.max(velocities))
        ...
        if portsFlag == True:
            ...
            maxPortsSpeedn = np.ceil(np.max(portsDt[:, 5]))
            # consVelocities[len(consVelocities) - 1]
            consVelocitiesPorts = np.arange(minPortsSpeedn, np.round(maxPortsSpeedn) + 1)

            stepVelRanges = int(np.round(len(consVelocitiesPorts) / 4))
            consVelocitiesRanges = []
            for i in range(0, len(consVelocitiesPorts), stepVelRanges):
                consVelocitiesRanges.append(consVelocitiesPorts[i])

            consVelocitiesRanges.append(
                consVelocitiesPorts[len(consVelocitiesPorts) - 1]) if consVelocitiesRanges.__len__() < 4 else \
                consVelocitiesRanges
            workbook._sheets[3]['B2'] = meanDraftPorts
            ...
            try:
                ...
                print('EXCEPTION IN WEIGHTS SWELL Ports')
            ...
            try:
                ...
                print('EXCEPTION IN WEIGHTS WS Ports')
            ...
            try:
                ...
                print('EXCEPTION IN WEIGHTS WD Ports')
                print(str(e))
            ...
            speedFoc = np.array(
                [k for k in portsDt if
                 (k[5] >= 5 and k[5] <= maxPortsSpeedn)  and k[8] > 1])
            ...
            consVelocitiesPorts = np.arange(np.round(minPortsSpeedn), np.round(maxPortsSpeedn), )

            #maxPortsSpeedn = maxPortsSpeedn if maxPortsSpeedn <= consVelocities[len(consVelocities) - 1] else \
            #consVelocities[len(consVelocities) - 1] - 1

            consVelocitiesJSON = np.arange(np.round(minPortsSpeedn), np.round(maxPortsSpeedn), 0.5)
            stepVelRanges = int(np.round(len(consVelocities) / 4))
            consVelocitiesRanges = []
            for i in range(0, len(consVelocitiesPorts), 1):
                consVelocitiesRanges.append(consVelocitiesPorts[i])

            #consVelocitiesRanges.append(
                #consVelocitiesPorts[len(consVelocities) - 1]) if consVelocitiesRanges.__len__() < 4 else None
                ...
                for w in range(0, len(windF) - 1):
                    for s in range(0, len(swellH) - 1):
                        ...
                        for i in range(0, len(wind) - 1):
                            ...
                            numberOfApp11_8.append(arrayFoc.__len__())  # + centralArray.__len__())
                            '''rawFoc = np.array([k for k in portsDt if (k[5]>= stw - 0.25 and k[5]<=stw+0.25) and
                            ...
                            if (s > 0 and w >= 0):
                                cellValue = round(
                                    (portsDt7801[len(portsDt7801) - 5] + (
                                                portsDt7801[len(portsDt7801) - 5] * swellHeightWeights[s])),
                                    2)
                            elif s == 0 and w == 0:
                                ...
                                if w < 3:
                                    cellValue = round(
                                        portsDt7801[len(portsDt7801) - 40] + (
                                                    portsDt7801[len(portsDt7801) - 40] * (windForceWeights[w])), 2)
                                elif w == 3:
                                    cellValue = round(
                                        portsDt7801[len(portsDt7801) - 40] + (
                                                portsDt7801[len(portsDt7801) - 40] * (windDirWeights[i])), 2)
                                elif w > 3:
                                    cellValue = round(
                                        portsDt7801[len(portsDt7801) - 40] + (
                                                portsDt7801[len(portsDt7801) - 40] * (windForceWeights[w])), 2)
                            ...
                            outerItem['cells'].append(item)
                            portsDt7_8.append(cellValue)
                            portsDt7801.append(cellValue)

                        if consVelocitiesJSON[vel] - int(consVelocitiesJSON[vel]) == 0:
                            for i in range(row[w], row[w] + 5):
                                try:
                                    workbook._sheets[3][column[s - 1 if s == 8 else s] + str(i)] = str(
                                        portsDt7_8[i - row[w]])  # + '(' + str(numberOfApp11_8[i - row[w]]) + ')'
                                    workbook._sheets[3][column[s - 1 if s == 8 else s] + str(i)].alignment = Alignment(
                                        horizontal='right')
                                except:
                                    print("Exception")
                    lastLenportsDt7801 = len(portsDt7801)
                json_decoded['ConsumptionProfile']['consProfile'].append(outerItem)
            # workbook.save(filename=pathToexcel.split('.')[0] + '_1.' + pathToexcel.split('.')[1])
            # return
            ladenSPEEDMin = portsDt7_8
            ####################END 8 SPEED ########################################################################
            ####################END 8 SPEED #######################################################################
                ...
                for w in range(0, len(windF) - 1):
                    for s in range(0, len(swellH) - 1):
                        ...
                        for i in range(0, len(wind) - 1):
                            ...
                            # cellValue = meanRawFoc
                            '''lstmPoint =[]
                            pPoint = np.array([meanDraftPorts,(wind[i]), (windF[w]+windF[w+1])/2,consVelocitiesJSON[vel], (swellH[s])])

                            #lstmPoint.append(np.array(
                                    #[meanDraftPorts, (wind[i]), ((windF[w] + windF[w + 1]) / 2) - 1,
                            ...
                            for k in np.arange(0,n_steps):
                                    ...
                                    lstmPoint.append(np.array(
                                        [meanDraftPorts, (wd[k]), wf[k],
                                         stw[k], (swh[k])]))
                                    #lstmPoint.append(np.array(
                                            #[meanDraftPorts, (wd[k]), windF[startW]+ countW,
        ...
        if ladenFlag == True:
            ...
            try:
                ...
                '''p2_01 = p2.predict(swell01.reshape(-1,1))
                p2_12 = p2.predict(swell12.reshape(-1,1))
                p2_23 = p2.predict(swell23.reshape(-1,1))
                p2_34 = p2.predict(swell34.reshape(-1,1))
                p2_45 = p2.predict(swell45.reshape(-1,1))
                p2_56 = p2.predict(swell56.reshape(-1,1))
                p2_67 = p2.predict(swell67.reshape(-1,1))
                p2_78 = p2.predict(swell78.reshape(-1,1))'''
                ...
                '''factorSWH = [0, np.mean(abs(p2_12 - p2_01) / p2_01),
                            np.mean(abs(p2_23 - p2_12) / p2_12),
                            np.mean(abs(p2_34 - p2_23) / p2_23),
                            np.mean(abs(p2_45 - p2_34) / p2_34),
                            np.mean(abs(p2_56 - p2_45) / p2_45),
            ...
            speedFoc = np.array([k for k in speedFoc if k[8] >= (meanFoc - (2.5 * stdFoc)) and k[8] <= (meanFoc + (2.5 * stdFoc))])'''
                    ...
                    for s in range(0, len(swellH) - 1):
                        ...
                        for i in range(0, len(wind) - 1):
                            ...
                            if (s > 0 and w >= 0):
                                ...
                                if w < 3:
                                    cellValue = round(
                                        ladenDt7801[len(ladenDt7801) - 40] + (ladenDt7801[len(ladenDt7801) - 40] * (windForceWeights[w])), 2)
                                elif w==3:
                                    cellValue = round(
        ...
        if ballastFlag == True:
            ...
            stdFoc = np.std(speedFoc[:, 8])
            speedFoc = np.array(
                [k for k in speedFoc if k[8] >= (meanFoc - (3 * stdFoc)) and k[8] <= (meanFoc + (3 * stdFoc))])'''
                                ...
                                if (s > 0 and w >= 0):
                                    cellValue = round(
                                        (ballastDt7801[len(ballastDt7801) - 5] + (
                                                    ballastDt7801[len(ballastDt7801) - 5] * swellHeightWeights[s])),
                                        2)
                                elif s == 0 and w == 0:
                                    cellValue = round(centralMean)  # + (centralMean * windDirWeights[i]), 2)
